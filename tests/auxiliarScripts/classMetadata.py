...
with open('output.txt', 'w', encoding='utf-8') as file:
    ...
    for n, row in enumerate(sheet.iter_rows(min_row=2), start=2):
        ...
        for col_idx in range(8):  # A=0, B=1, ..., H=7
            ...
            cell_value = row[col_idx].value if row[col_idx].value is not None else ""
            
            # Formata a string e escreve no arquivo
        ...
        if row[8].value is not None:  # Se a célula In não estiver vazia
            ...
            for part in parts:
                ...
                alt_label_line = f"{objectName} skos:altLabel \"\"@en .\n"
                file.write(alt_label_line)

print("Processamento concluído. Resultados salvos em output.txt.")